...
namespace cartservice.cartstore
{
    public class RedisCartStore : ICartStore
    {
		...
		// Configure exporter to export tarces to Zipkin
		...
		//     .Build();
		string zipkin_addr = Environment.GetEnvironmentVariable("ZIPKIN_ADDR");
		string jaeger_addr = Environment.GetEnvironmentVariable("JAEGER_ADDR");
		string jaeger_port = Environment.GetEnvironmentVariable("JAEGER_PORT");
		if ( zipkin_addr != null )
		{
		    Sdk.CreateTracerProviderBuilder()
		        ...
		        {
		            o.ServiceName = "cartservice";
		    	    o.Endpoint = new System.Uri(zipkin_addr);
		        })
		        .Build();
                    Console.WriteLine("initialization zipkin exporter completed");
		}
		else if ( jaeger_addr != null )
		{
		    Sdk.CreateTracerProviderBuilder()
                        .AddRedisInstrumentation(redis)
			.SetResourceBuilder(ResourceBuilder.CreateDefault().AddService("cartservice"))
                        .AddJaegerExporter(o =>
                        {
                            o.AgentHost = jaeger_addr;
                            o.AgentPort = int.Parse(jaeger_port);
                        })
                        .Build();
                    Console.WriteLine("initialization jaeger exporter completed");
		}
		else
		{
                    Console.WriteLine("initialization tracing disabled");
		}
